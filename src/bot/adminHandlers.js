...
const { createSubscription, syncUserStatusFromSubscriptions, extendActiveSubscriptionsByDays } = require('../services/subscriptionService');
...
const registerAdminHandlers = (bot) => {
  ...
  const runModifyPlanConfirmed = async (ctx, payload) => {
    ...
    try {
        ...
        const runExtendDaysConfirmed = async (ctx, payload) => {
          ...
          try {
            ...
            if (!result.matchedCount) {
              return ctx.reply(`ℹ️ No active subscriptions found for category *${escapeMarkdown(result.category)}*.`, { parse_mode: 'Markdown' });
            }
      ...
      await ctx.reply(`❌ Failed to extend active subscriptions.${ escapeMarkdown(err.message || 'Please try again.')
          }`, { parse_mode: 'Markdown' });
    }
  };
      }
...
bot.action(/^adm_conf_([a-f0-9]{32})$/i, requireAdmin, async (ctx) => {
  ...
  if (entry.actionType === 'extend_days_all') {
    return runExtendDaysConfirmed(ctx, entry.payload);
  }
});
...
bot.command('expiries', requireAdmin, async (ctx) => {
  try {
    ...
    // ── /days (movie|desi|non_desi)|(single_id|multi_id|all)|count — extend active subscriptions ──
    bot.command('days', requireAdmin, async (ctx) => {
      try {
        const raw = String(ctx.message?.text || '').replace(/^\/days(?:@\w+)?\s*/i, '').trim();
        if (!raw) {
          return ctx.reply(
            'Usage: `/days movie|single_id|<telegramId>|<days>\n/days movie|multi_id|<id1,id2,...>|<days>\n/days movie|all|<days>`',
            { parse_mode: 'Markdown' }
          );
        }

        const parts = raw.includes('|')
          ? raw.split('|').map((value) => String(value || '').trim())
          : raw.split(/\s+/).map((value) => String(value || '').trim());

        const [categoryInput, modeInput, targetInput, daysInput] = parts;
        const normalizedCategory = parseCategoryInput(categoryInput);
        if (!normalizedCategory) {
          return ctx.reply('❌ Invalid category. Use movie, desi, or non_desi.');
        }
        ...
        const isSingleMode = ['single_id', 'single', 'singleid'].includes(mode);
        const isMultiMode = ['multi_id', 'multi', 'multiid'].includes(mode);

        if (!isAllMode && !isSingleMode && !isMultiMode) {
          return ctx.reply('❌ Invalid mode. Use single_id, multi_id, or all.');
        }

        const daysToken = isAllMode ? targetInput : daysInput;
        const targetToken = isAllMode ? null : targetInput;
        const extraDays = parseInt(String(daysToken || '').trim(), 10);
        if (!Number.isInteger(extraDays) || extraDays <= 0) {
          return ctx.reply('❌ Invalid days count. Use a positive integer like 1, 2, 3, 4, 5.');
        }

        if (isAllMode) {
          const summary =
            `This will extend *all active* *${escapeMarkdown(normalizedCategory)}* subscriptions by *${extraDays} days*.`;

          await promptAdminButtonConfirmation(ctx, {
            actionType: 'extend_days_all',
            payload: {
        ...
        if (isSingleMode) {
          const targetId = parseInt(String(targetToken || '').trim(), 10);
          if (!targetId) {
            return ctx.reply('❌ Invalid telegramId. Usage: /days movie|single_id|<telegramId>|<days>');
          }
          ...
          if (!currentSub) {
            return ctx.reply(
              `ℹ️ No active subscription found for user \`${targetId}\` in category *${escapeMarkdown(normalizedCategory)}*.`,
              { parse_mode: 'Markdown' }
            );
          }

          const previousExpiry = new Date(currentSub.expiryDate);
          const newExpiry = addDays(previousExpiry, extraDays);
          ...
          return ctx.reply(
            `✅ Extended user \`${targetId}\` in *${escapeMarkdown(normalizedCategory)}* by *${extraDays} days*.` +
        ...
        const targetIds = String(targetToken || '')
          .split(/[,\s]+/)
          .map((value) => parseInt(value, 10))
          .filter((value) => Number.isInteger(value) && value > 0);

        if (!targetIds.length) {
          return ctx.reply('❌ Provide one or more telegram IDs. Usage: /days movie|multi_id|<id1,id2,...>|<days>');
        }

        const result = await extendActiveSubscriptionsByDays({
          category: normalizedCategory,
          days: extraDays,
          telegramIds: targetIds,
        });

        const matchedIdSet = new Set(result.matchedTelegramIds.map((value) => String(value)));
        const missingIds = targetIds.filter((value) => !matchedIdSet.has(String(value)));
        ...
        return ctx.reply(
          `✅ Extended * ${escapeMarkdown(result.category)
          }* active subscriptions by * ${result.days} days *.` +
          `\nUpdated subscriptions: * ${result.modifiedCount}* ` +
          `\nMatched users: * ${result.matchedTelegramIds.length}* ` +
          (missingIds.length ? `\nNo active subscription found for: ${missingIds.map((value) => `\`${value}\``).join(', ')} ` : ''),
          { parse_mode: 'Markdown' }
        );
      } catch (err) {
        logger.error(`days command error: ${err.message} `);
        await ctx.reply(`❌ Failed to extend subscription days.${escapeMarkdown(err.message || 'Please try again.')} `, { parse_mode: 'Markdown' });
      }
    });
